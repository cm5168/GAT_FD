...
class ProcessWindow(QWidget):
    ...
    def gatp_run_process(self):
        ...
        for i in range(fnc_pro_filelength):
            ...
            if self.file_format_dropdown.currentIndex() == 0:
                # Load NIfTI with memory mapping, memory-efficient per-timepoint extraction
                func_img = nib.load(file_path, mmap=True)
                if isinstance(atlas_img, np.ndarray):
                    atlas_data = atlas_img
                else:
                    atlas_data = nib.load(atlas_img).get_fdata().astype(np.int32)
                # Resample atlas if needed to match functional image shape
                if atlas_data.shape != func_img.shape[:3]:
                    from nilearn.image import resample_to_img
                    atlas_nii = nib.Nifti1Image(atlas_data, affine=func_img.affine)
                    resampled = resample_to_img(atlas_nii, func_img, interpolation='nearest')
                    atlas_data = resampled.get_fdata().astype(np.int32)
                n_timepoints = func_img.shape[3]
                roi_labels = np.unique(atlas_data)
                roi_labels = roi_labels[roi_labels > 0]
                atl_len = len(roi_labels)
                fnc_raw_len = n_timepoints
                fnc_window_count = fnc_raw_len - fnc_window_size + 1

                # Efficient, low-memory ROI extraction: process one timepoint at a time
                flat_atlas = atlas_data.flatten()
                from scipy import ndimage
                atlased_data = np.zeros((n_timepoints, atl_len), dtype=np.float32)
                # Efficient block-wise approach: process in small batches of timepoints
                block_size = 10  # Tune this for your RAM (10 is safe for most systems)
                for start in range(0, n_timepoints, block_size):
                    end = min(start + block_size, n_timepoints)
                    # Only load a small block into RAM
                    block = np.asarray(func_img.dataobj[..., start:end], dtype=np.float32)
                    block_2d = block.reshape(-1, end - start)
                    for idx, roi in enumerate(roi_labels):
                        mask = (flat_atlas == roi)
                        if np.any(mask):
                            roi_ts = block_2d[mask, :]
                            atlased_data[start:end, idx] = np.mean(roi_ts, axis=0)
                        else:
                            atlased_data[start:end, idx] = 0.0
                ...
                fnc_raw_len = atlased_data.shape[0]
                fnc_window_count = fnc_raw_len - fnc_window_size + 1
